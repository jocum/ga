...
type ChromosomeGroup []*bl.Bl

// 定义 遗传计算基础属性
type GeneticAlgorithm struct {
	Options Options
	// 初始个体
	AdamChromosome *bl.Bl
	// 初始个体基因数量
	AdamGeneNum int
	// 染色体族群
...
// 初始化 遗传对象
func NewGeneticAlgorithm(adam *bl.Bl, opts ...Option) *GeneticAlgorithm {
	// 加载配置
	os := newOptions(opts...)
	// 基因长度
...
func (ga *GeneticAlgorithm) Adaptability() {
	...
	for _, v := range ga.ChromosomeGroup {
		go func(chromosome *bl.Bl) {
			// 拼版前 初始化箱子
			chromosome.Boxs = make([]*bl.Box, 0)
			// 实际拼版
			chromosome.Packing()
			// 计算适应度
...
*/
func (ga *GeneticAlgorithm) CutGene(genes bl.Rects) map[int]bl.Rect {
	genePart := make(map[int]bl.Rect)
	// 基因交换就是切片的位置替换由于每个基因都是独一无二的无法直接的交换 只对其位置做操作
	// 这里先获取切片需要获取数量之前的随机数，通过该随机数为起点窃取Ga.CrossGeneNum个基因
	randNum := bl.RandInt(ga.AdamGeneNum - ga.CrossGeneNum)
...
/*
	@description 交叉基因
*/
func crossGene(genes bl.Rects, genePart map[int]bl.Rect) {
	// 循环基因片段
...
*/
func (ga *GeneticAlgorithm) Mutation(chromosome *bl.Bl) {
	// 根据基因变异比例 循环变异基因
	for i := 0; i < ga.MutationGeneNum; i++ {
		// 获取随机数
		randNum := bl.RandInt(ga.AdamGeneNum)
		// 对当前索引的基因进行变异
...
/*
	@description  迭代
		根据迭代次数、完成条件迭代
*/
func (ga *GeneticAlgorithm) Iterator() *bl.Bl {
	// 先初始化获取生成族群
